...
class ConnectionResource(object):
    ...
    def __init__(self, queue, conn,
                 ...
                 timeout=1, ssl=dict(), _handshake_version=10, **kwargs):
        self._queue = queue
        if conn:
            self._conn = conn
        else:
            self._conn = r.connect(host, port, db, auth_key, user, password, timeout, ssl, **kwargs)
...
class RethinkPool(object):
    ...
    def get_resource(self):
        ...
        if self._queue.empty() and self.current_conns < self._queue.maxsize:
            ...
            try:
                logger.info("tried the connection")
                r.expr(1).run(conn)
